...
| design.md 决策 | 实现位置 |
|---|---|
| 渠道实体（平台/凭证/启停） | `internal/channel/domain` + `application` + `infrastructure/persistence` |
| 凭证 AES-GCM + masked 回显 | `internal/platform/crypto` + `domain/credential.go` |
| 菜单按渠道作用域（order 替代 row/col） | `internal/menu`（`channel_menu_*` 表，`(channel_id, id)` 复合主键） |
| 会话地址渠道化 | `sharedkernel.ChatID` + `ChannelAddr`；`sessions.channel_id + chat_external_id` |
| 端口契约 + TG 端口实现 | `internal/channel/ports` + `internal/channel/tg` |
| 热生效装配器 | `internal/channel/runtime`（5s watch、start/stop/restart、退避重试） |
| 通知按渠道投递 + 幂等 | `runtime.NotifyRouter` + `tg.Adapter.notified` |
...
| channel-menu-config（5 需求 12 场景） | 通过：渠道隔离（含跨渠道同 ID 回归测试）、中立模型无 TG 字段、extras 按渠道存取与无效忽略、Case 挂载/反查、默认种子 |
| channel-runtime-ports（4 需求 7 场景） | 通过：端口契约、应用层无 SDK 依赖、身份渠道化、通知按渠道+幂等 |
...
| admin-web-shell（3 需求 5 场景） | 通过：侧栏渠道替代主键盘、顺序正确、中英文案 |

### 2.5 proposal 目标已满足 — PASS

渠道一级实体、菜单收进渠道详情、中立领域 + extras、运行时端口化、身份渠道化、旧路径/旧表/旧字段物理删除，均已实现；无存量数据迁移要求满足。
...
- CRITICAL：菜单项 ID 全局主键冲突导致多渠道无法共存 → 复合主键 `(channel_id, id)` + 链接表渠道化 + 回归测试（`TestReplaceTree_SameSeedAcrossChannels`）
- IMPORTANT：中立领域残留 TG 3500 上限 → 移入 TG messenger 截断（4096/1024 字符）
- IMPORTANT：putExtras 值拷贝导致 menu_item_id 归一失效 → 索引循环 + 测试
- IMPORTANT：渠道停用后 notify handler 未注销 → `registry.unset`
...
4. **装配器持锁执行 Start/Stop**：会短暂阻塞同轮其他渠道 reconcile，当前规模可接受。
5. **`notified` 去重 map 无上限**：随任务数增长，TG bot 生命周期内可接受，后续可加 LRU。
6. **SendMenu fallback 种子使用 "default" 渠道**：仅菜单读取失败时兜底，不影响正常路径。
