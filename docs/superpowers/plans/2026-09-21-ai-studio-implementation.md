...
- [ ] 先写用例测试：发送消息自动建会话、首条输入生成标题、后台 Run 生命周期、取消、重试和断线后回读。
- [ ] 实现 SessionService、AssetService、FlowService、RunService。
- [ ] 建立后台 Runner 与可恢复事件流；HTTP 请求结束不取消运行。
- [ ] 实现三档权限策略：请求批准、帮我批准、完全访问。
- [ ] 实现审批挂起/通过/拒绝与可恢复执行。
- [ ] 运行并发、状态迁移与恢复测试。

## Task 3：Mock Agent 与 Mock 工作流闭环

- [ ] 先写端到端服务测试，输入漫画需求后应产生助手回复、工作流 Run、输入资产、输出图片资产、Flow 节点与 Trace。
- [ ] 实现可注入的 Agent/Workflow 接口。
- [ ] 实现确定性 Mock Agent、Mock「分镜生成」工作流与本地 SVG/图片产物。
- [ ] 验证资产内容可通过现有 Blob Store 读取。
- [ ] 验证后台运行时切出页面不影响完成。
