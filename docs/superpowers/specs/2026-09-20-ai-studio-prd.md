...
Studio 不建立独立视觉体系，也不引入第二套通用 UI 组件库。

- 页面壳、导航、表单、按钮、弹层、反馈、列表和设置页复用 Pixoma 后台现有组件与交互模式。
- Chat 消息、流式状态、Composer 和 Agent Tool UI 使用 assistant-ui，但必须套用 Pixoma 语义令牌和组件形状，不能直接保留第三方默认主题。
- Flow 使用平台现有的 `@xyflow/react`；节点内部继续使用 Pixoma UI 组件。
- 新组件先判断能否由 `web/admin/src/components/ui` 组合。确实缺失时，只通过项目的 shadcn CLI 补充到同一组件目录。
- 禁止复制一套 Studio 专用 Button、Dialog、Select、Toast、Form 或主题变量。
...
- 后台运行、断线恢复和 Trace。
- 资产版本、来源和固定引用。
- 与现有 Pixoma 后台一致的设计系统、组件、主题和文案。
- 本期范围内所有页面、状态、反馈、响应式、键盘与无障碍交互的生产级实现。
...
- Studio 与现有 Pixoma 后台使用同一套 shadcn/ui 组件、语义令牌、字体、圆角、间距和交互反馈。
- assistant-ui 只承担 Chat 运行时与 Chat primitives，最终视觉不出现第三方默认样式割裂。
- 不存在 Studio 专用的重复基础组件、裸色值、临时主题或演示级 CSS。
...
- 加载、空态、错误、无权限、断线重连、禁用、后台运行、等待批准、取消和成功状态全部具备明确界面与恢复动作。
- 桌面与窄屏布局、键盘操作、可见聚焦、触屏命中区和 reduced motion 达到 UX 文档定义。
- 本期验收项全部达到，不以占位界面、静态假数据、未接线按钮或后续补齐状态交付。
...
- 前端实现清单明确映射到现有 Pixoma 组件；缺失组件已经确认采用 shadcn CLI 补充，不新增通用 UI 库。
- UI 验收以“本期范围 100% 可用”为准：主流程、边界状态、错误恢复、响应式和无障碍必须同批交付。
