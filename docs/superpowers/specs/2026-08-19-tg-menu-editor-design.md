...
交互框架（channel-interaction-framework）已定义方向：菜单 = 能力入口树、主键盘显式配置（≤6）、一层分组、管理端「选能力 → schema 参数表单 → 消息平台形态预览」。本次按 Case 编辑器改造的思路落地为具体交互。
...
| 项 | 选择 |
|---|---|
| 布局 | 所见即所得双栏：左 TG 手机模拟（实时、可点击），右配置面板（当前选中项）；顶栏 = 消息平台名 + 启用 + 保存（右上角） |
