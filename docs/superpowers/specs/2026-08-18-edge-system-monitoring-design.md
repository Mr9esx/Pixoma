...
- Round 6 图表样式（2026-08-18 build，任务 3.20）：用户要求从 Total Revenue 卡样式改为 MHTML「Sprint health」卡样式——卡片 `p-4` + 标题 `h2.text-base.font-semibold` + 主体 `lg:grid-cols-[minmax(0,1fr)_92px]`（左图右值），右侧统计列 `grid-cols-3 … lg:grid-cols-1 lg:text-right`，值 `text-xl/leading-6 font-semibold`（首项）与 `text-lg`（其余）、说明 `text-muted-foreground text-[11px]`。占用率系列（CPU/内存/GPU/显存）右侧显示「当前 / 最高 / 平均」百分比；内存/显存已用字节仍作为占用率图辅助线，仅进图例与 tooltip；I/O 读/写各显示「当前 / 最高 / 平均」（`formatBytes` 人类可读单位）。图高维持 Round 3 的 50% 约定（`CHART_HALF_HEIGHT`），内边距与 tooltip 格式维持 Round 4/5 约定。新增纯函数 `seriesStats(values, format)`（过滤 null，返回 current/max/avg 格式化值）并单测。
