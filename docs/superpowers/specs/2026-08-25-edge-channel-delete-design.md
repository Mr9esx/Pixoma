...
# 计算节点与渠道删除：确认制清理式删除 深度技术设计

## 1. 目标

对齐工作流（Case）删除的交互模式，把计算节点（Edge）与渠道（Channel）的删除改为「**先展示真实状态，再二次确认，确认后自动清理**」：

- **计算节点**：补充前端删除入口；删除时把该节点 running 任务终态标记失败（原因「节点已删除」）并通知发起用户；有任务时需用户勾选确认。
- **渠道**：删除不再要求先停用；删除时在同一事务内删除渠道行、终止该渠道活跃会话（标记 exited）并通知会话用户、清理该渠道菜单/卡片行；弹窗先展示该渠道进行中会话数与执行中/排队任务数，有影响时需勾选确认。
...
- 「必须先停用」对用户不友好，删除一个启用中的渠道本可以靠确认弹窗完成。
- 错误文案写「active sessions/tasks」，实际只查会话；执行中/排队任务（会话已 submitted）并不会被统计，与文案不符。
- 删除渠道不清理 `channel_main_menus` / `channel_cards` 孤儿数据。
- 活跃会话未终止，用户会被悬挂在流程中。
...
- `Delete` 变为：`Store.Get`（404）→ 调 `DeleteWithCleanup` → commit 后对返回的 chat 发布 `UserNotify{Kind: "session_terminated", ErrorMsg: "该渠道已被管理员删除，当前会话已结束。"}`（best-effort）。
...
- **渠道进行中会话数**：sessions 管理 API 增加 `channel_id` 过滤（`SessionRow` 已有该列，仿 case_id 实现）。
- **渠道执行中/排队任务数**：tasks 管理 API 增加 `channel_id` 过滤；`TaskRow` 无 channel 列，通过 `JOIN sessions ON sessions.id = tasks.session_id` 过滤 `sessions.channel_id`。
...
- 删除弹窗内容（打开时实时拉取）：
  - 「M 个进行中会话将被终止并通知用户」；
  - 「K 个执行中/排队任务将继续运行，但渠道删除后完成通知无法送达」；
...
渠道：详情页删除按钮（不再因 enabled 禁用）
  → 实时拉取进行中会话数 + 执行中/排队任务数
  → 弹窗展示 + 勾选确认
  → DELETE /api/v1/channels/{id}
  → channelapp.Service.Delete（事务：删渠道行 + 会话 exited + 清理菜单/卡片）
...
| 节点不存在 | 404 |
| 渠道删除 | `DELETE /api/v1/channels/{id}`；成功 200 `{"deleted": true}`；不再有启用/会话 409 |
| 渠道不存在 | 404 |
...
复用 `UserNotify.Kind = "session_terminated"`（TG adapter 已支持，渲染 `ErrorMsg`）；渠道场景 ErrorMsg 为「该渠道已被管理员删除，当前会话已结束。」。
...
- 通知失败：记日志，不阻断。
- 渠道删除后运行中任务：继续执行（job_ref 快照），但完成通知因渠道不存在无法送达（路由会跳过并记日志）；弹窗已提前告知用户。
...
- channels 服务测试：enabled 渠道可直接删；事务清理（会话 exited、菜单/卡片删除、返回 chat）；无清理注入时退化。
- sessions/tasks handler 测试：`channel_id` 过滤。
- 前端 contract 测试：节点详情页删除入口与影响文案；渠道删除按钮不再禁用、弹窗影响文案；i18n 成对。

## 8. 不做的事（YAGNI）

- 不停远程 agent 进程（无法从控制面保证；仅提示）。
- 不级联清理节点 metrics 历史（保留可查）。
- 渠道删除不中断执行中/排队任务（与 workflow 一致）。
- 不新增「渠道任务数」专用聚合接口（复用 tasks 列表的 channel_id 过滤）。
