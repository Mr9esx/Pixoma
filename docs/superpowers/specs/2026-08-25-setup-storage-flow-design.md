...
- 步骤文案：Title「文件存储配置」、Desc「决定了生成的图和视频存放的位置。」；`localfs` 选中时展示 warn Alert（amber 系，`Alert` 组件新增 `warn` 变体 + `CircleAlert` 图标，Title「注意！」、Description 放两行文案：`<br />` 分隔「这个配置只适合 ComfyUI 和后台在同一台机器上使用，」/「无法使用远程节点。」）；驱动标签「S3 / 火山 TOS / 本机目录」（设置页 i18n `blobS3` 同步为「S3」）。
