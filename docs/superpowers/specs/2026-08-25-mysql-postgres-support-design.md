...
- 数据库步骤拆两个按钮：「连通性测试」调 `POST /api/v1/setup/database` 并显示结果（通过以 success Alert 显示「连接正常」，失败以 destructive Alert 显示错误），「继续」始终可点；连接配置变更后 `dbTested` 自动失效。`Alert` 组件新增 `success` 变体（emerald 系）。
