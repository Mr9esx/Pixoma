...
系统 MUST 在管理后台提供初始化向导，引导用户完成至少：业务数据库、对象存储、执行节点/Comfy 指引、消息平台（含 Telegram Bot Token）等步骤；业务数据库步骤 MUST 支持 `sqlite`、`mysql`、`postgres` 三种驱动；每步 MUST 做可达性或合法性校验，失败时 MUST 给出可诊断错误；对象存储步骤 MUST 提供连通性测试，MUST 支持共享目录（SMB/NFS 挂载）选项。

#### Scenario: 本机路径完成向导
- **WHEN** 用户配置可用业务库与 localfs 目录，并完成节点与消息平台必要项
