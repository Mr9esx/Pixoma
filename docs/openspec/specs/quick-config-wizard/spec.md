...
#### Scenario: 未启用渠道不可配置
- **WHEN** 用户尝试配置未启用渠道的投放
- **THEN** 该渠道卡片置灰并提示先启用渠道

### Requirement: 完成页就绪清单与发布门禁
完成页 MUST 展示三项就绪状态：工作流已导入（`bindings.workflow`、`inputs`、`input_schema` 完整）、处理流程已配置（至少一条规则且每个使用中的 Topic 已绑定启用节点）、至少一个渠道投放；状态 MUST 以三态展示（就绪 / 警告 / 缺口）。存在缺口时 MUST 禁用发布按钮、列出缺口，并允许从完成页回跳到对应步骤补齐；全部就绪后发布按钮 MUST 可用，发布操作 MUST 将 Case 设为启用。
