...
Edge-Agent MUST 支持配置订阅 Topic 列表（如 `subscribe_topics`）；未配置时 MUST 不消费任何 Topic（不绑定调度通道则无法接收任务）。进程启动后 MUST 按实际订阅集合向控制面申报并长轮询领取；控制面 presence 响应 MUST 返回节点是否在消费，未消费时管理端 MUST 展示提醒。Edge MUST NOT 解析投放表达式（规则求值只发生在控制面）。

#### Scenario: 未配置订阅不消费
- **WHEN** Edge 配置未声明 subscribe_topics
- **THEN** Edge 不绑定任何 Topic，不领取任务；presence 返回 consuming=false，管理端提示需要绑定调度通道
