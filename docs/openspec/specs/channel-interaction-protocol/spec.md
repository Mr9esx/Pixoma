...
## Purpose
统一交互协议定义消息平台上「用户点了一下」如何变成「业务执行并渲染结果」：适配器把 UI 事件翻译为能力调用，执行后按能力的消息平台渲染声明输出结果；适配器不再认识具体业务。
## Requirements
### Requirement: 事件 → 能力调用协议
系统 MUST 将消息平台 UI 事件（按钮点击、文本、媒体、返回）翻译为能力调用（capability_id + params + 消息平台账户上下文）；能力调用 MUST 携带执行上下文，其中用户身份为消息平台账户（消息平台 + 外部用户 id）。

#### Scenario: 按钮点击触发能力
- **WHEN** 用户点击 TG 主键盘或消息按钮
- **THEN** 适配器翻译为对应能力调用并执行，返回结果按消息平台渲染

### Requirement: 结果渲染协议
能力执行结果 MUST 以消息平台无关的结果结构返回（文本/选项列表/媒体/错误），由适配器按本消息平台渲染声明输出；同一结果在不同消息平台可呈现不同形态。

#### Scenario: 结果跨消息平台形态不同
- **WHEN** 同一能力返回选项列表
