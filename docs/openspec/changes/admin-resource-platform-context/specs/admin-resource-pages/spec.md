...
控制台 MUST 提供 User、Session、Task 的列表与详情页；Task 页 MUST 支持取消等已由 API 提供的运维动作。Task 列表 MUST 展示消息平台、关联用户和关联 Session；Session 列表 MUST 展示消息平台；User 列表 MUST 展示消息平台和统一的「用户信息」列，且 MUST NOT 展示 Telegram 专用 ID 列或专用筛选字段。
