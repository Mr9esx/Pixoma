...
Session DTO 直接暴露持久化层已有的 `channel_id`；Session 领域模型补充该只读上下文字段并从行映射填充。
...
三个列表沿用现有 DataTable 和 Master–Detail 结构。Task 增加「消息平台 / 用户 / Session」列，Session 增加平台列，User 用平台列加用户信息列替换 TG 专用列。列名与值统一走 i18n；字段缺失时显示空态而不是回退到平台猜测。
