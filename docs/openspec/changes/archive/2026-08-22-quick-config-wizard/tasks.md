...
- [x] 5.2 消息平台卡片：启用消息平台可配置（锁定 `open_workflow` + direct/list），未启用置灰
- [x] 5.3 入队待提交菜单（消息平台 + 标签 + 模式），完成页统一读菜单 → 追加 → `PUT` 全量写回
...
- [x] 8.4 端到端冒烟：导入工作流建 Case → Step 2 画布配置规则/Topic/节点 → 投放消息平台 → 完成页全绿 → 发布 → 回读一致
