...
管理后台要把「一个工作流从创建到真正被用户使用」配通，需要横跨 Case 编辑、任务分流画布（React Flow）、Topic/计算节点管理、消息平台菜单四个页面，操作链路长且管理员不知道「还差什么」。需要一个菜单级的「快速配置」入口：用 **Formity 的 flow 步骤机制**（参照其官方 examples 形态）编排三步向导——库一次渲染一个步骤屏幕，配合完成页就绪清单与发布门禁，让不熟悉内部结构的运营能明确知道下一步该做什么。
...
- **三步由 Formity flow 步骤机制驱动**（参照 Formity examples：一次只渲染当前步骤屏幕、进度指示「Step X of 3」、`next`/`back`/`jump` 切换、已完成步骤可回跳；不做独立路由页、不堆叠在同一页面）：
  ...
  - **Step 3 投放**：独立页面复用 `MenuPlacementsSection` 与 `action-form`（锁定 `open_workflow`），配置消息平台菜单入口。
