...
- [x] 1.8 agent 上报 `started_at`（首次心跳时间，进程内只记一次）与 `comfy_version`（解析 `/system_stats` 的 `comfyui_version`）；`SystemStats` 新增 `ComfyUIVersion` 字段并补测试
...
- [x] 2.6 `edges` 表新增 `started_at` / `comfy_version` 列；`Record`/`EdgeRow`/`Repository.UpdatePresenceInfo`；agent presence handler 落库；admin DTO 返回两字段
...
- [x] 3.21 节点信息新增「启动时间」（创建时间下方）与「Comfy 版本」（分类上方）；类型/i18n/合同测试同步
...
- [x] 4.4 同步架构文档：`edges` 表 `started_at`/`comfy_version` 列与 presence 上报路径
