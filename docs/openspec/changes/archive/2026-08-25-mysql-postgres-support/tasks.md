...
- [x] 2.4 新增 `db-dsn.ts` 纯函数（sqlite/mysql/postgres 连接串组装）与单元测试
- [x] 2.5 向导数据库步骤改为结构化字段表单 + 「附加参数」输入框（直接追加任意 key=value）+ 切换驱动端口默认值联动，更新合同测试
- [x] 2.6 错误改用 `Alert variant="destructive"` 展示：常见数据库错误映射中文友好标题 + 实际错误详情（`db-error.ts` 与单元测试、合同测试）
- [x] 2.7 扩展错误映射边界场景：DNS/路由不可达、连接中断、连接数满、文件锁/磁盘、死锁、角色/表缺失、SSL/TLS、登录/密码、向导步骤前置、远程 localfs、代理、未授权等（单测扩至 19 用例）
- [x] 2.8 数据库步骤拆分「连通性测试」与「继续」按钮：测试通过显示连接正常，「继续」才可点，配置变更后需重新测连通（合同测试锁定）
- [x] 2.9 后端 `EnsureDatabase`：MySQL/Postgres 连服务器验证账号密码、目标库缺失自动创建（标识符转义），`setup.Handler.openDB` 统一接入，含单测与集成测试
- [x] 2.10 「继续」不要求先测连通（移除 `submitDisabled` 门控），连接问题在后续步骤以 Alert 报错（合同测试锁定）
- [x] 2.11 「连接正常」以 `Alert variant="success"` 展示并带 `CircleCheck` 图标（Alert 组件新增 success 变体），合同测试锁定
