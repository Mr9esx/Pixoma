...
| channel_id | TEXT NOT NULL | → channels.id |
| external_user_id | TEXT NOT NULL | 渠道作用域用户 id（TG 存字符串化的 tg user id） |
...
进程启动时扫描启用的渠道，为每个渠道启动对应适配器（独立轮询/长连接 goroutine，带 graceful stop）；渠道禁用触发对应适配器停止。`notifybridge` 从 tg 包泛化到 channel 层：通知携带渠道地址，按渠道路由到对应 `Messenger`。

### D7. 管理台与 API

- 侧栏「渠道」替代「主键盘」；渠道列表 → 新建向导（平台 + Token）→ 详情（基本信息 + 菜单配置 tab）
...
- [ChatID 类型迁移波及 conversation/task/notify 持久化与跨进程消息] → 分两步：先加渠道前缀语义与迁移脚本，再切换类型；旧值统一 `tg:` 前缀；notify JSON 兼容字段
- [凭证入库带来泄露面] → AES-GCM 加密 + masked 回显 + 文档警示；密钥经 env 注入
- [表结构替换引入回归] → 无存量数据，新 schema 全新建表；旧表模型代码随重构删除，测试覆盖等价行为
- [多适配器生命周期管理复杂] → 本期单 TG 渠道；装配器以注册表 + 启停接口收敛，为多渠道预留
...
1. 新增 `channels` 表与 `internal/channel` 领域；渠道由管理台创建，凭证仅存渠道
...
5. 新 API 上线并移除 `/api/v1/tg-menu`；管理台改版（渠道 → 菜单）
...
- 多渠道并行的真实优先级（本期仅单 TG）不影响本设计的数据模型预留
