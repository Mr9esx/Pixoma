...
- [x] 2.4 数据迁移：无存量数据，新表直接能力模型；dev 旧行按展示节点处理，由管理台重配（迁移脚本保留为设计项）
...
- [x] 4.1 主键盘：根层能力入口直达按钮 ≤6；超出进「更多」分组或消息按钮
- [x] 4.2 分组与流程：点分组 → 消息按钮列出子项与 open_case 入口；返回语义保留；每行按钮数按渲染声明
- [x] 4.3 open_case 行为等价回归：预览/开始/填表/确认/出图通知
- [x] 4.4 移除适配器内业务 switch（open_folder/open_case 等专属动作），改走 registry
...
- [x] 6.1 用户端交互：主键盘显式配置（≤6，平台不自动塞满）；超出入口限制提示
- [x] 6.2 用户端交互：分组最多一层（消息按钮内只有能力入口，无嵌套分组）
- [x] 6.3 用户端交互：流程每步提供返回/退出按钮（返回上一级或主菜单）
- [x] 6.4 管理端预览：按当前配置模拟渲染 TG 主键盘与消息按钮流程（遵循显式配置与一层分组规则）

## 7. 测试与回归

- [x] 7.1 单元：能力注册/参数校验/Result 结构/AccountCtx
- [x] 7.2 领域与持久化：菜单能力入口校验、params_json 往返、迁移脚本
- [x] 7.3 适配器：事件→能力调用翻译、主键盘 ≤6、一层分组消息按钮、返回语义
- [x] 7.4 API 与前端：菜单 API 载荷（capability_id）、编辑器 schema 表单、预览渲染
- [x] 7.5 全量 `go test ./...` + `pnpm test/build` + TG 手工回归（Case 流程等价 + 主键盘显式配置/一层分组/返回退出；真实 Token 回归待上线前）
