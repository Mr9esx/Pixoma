...
- 存储步骤文案：Title「文件存储配置」、Desc「决定了生成的图和视频存放的位置。」；选择 `localfs`（本机目录）时展示 warn Alert，文案两行展示（第一行「这个配置只适合 ComfyUI 和后台在同一台机器上使用，」第二行「无法使用远程节点。」）；对象存储驱动标签 S3/TOS/localfs 用「S3 / 火山 TOS / 本机目录」。
