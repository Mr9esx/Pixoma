...
- 不做巡检推送；查询热路径不打外部 Bot API。打开消息平台列表可异步踢一轮探测，不挡渲染。
...
探测若今天只活在详情页 React Query 里，组装器就读不到 — 要么让探测对组装器可见（例如随通道读模型提供上次结论），要么在读不到时 pending。实现期选一种，原则是「组装器看得见或明确未就绪」，不是「做一次通道表迁移需求」。`last_check` 为空视为未就绪：适配器 running 也不得当活路起点。
