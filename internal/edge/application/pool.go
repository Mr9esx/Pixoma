...
type PoolOptions struct {
	...
	AfterRefresh func(instances []edgedomain.Instance)
}

type pooled struct {
	record  edgedomain.Record
	healthy bool
}

// Pool keeps an in-process edge map keyed by instance id and implements
// edgedomain.Registry for scheduling.
type Pool struct {
	repo edgedomain.Repository
	opts PoolOptions
...
func NewPool(repo edgedomain.Repository, opts PoolOptions) *Pool {
	return &Pool{
		repo:         repo,
		opts:         opts,
...
func (p *Pool) SetAfterRefresh(fn func(instances []edgedomain.Instance)) {
	p.mu.Lock()
	defer p.mu.Unlock()
	p.afterRefresh = fn
}
...
func (p *Pool) ListHealthy(_ context.Context, filter edgedomain.CapabilityFilter) ([]edgedomain.Instance, error) {
	return p.list(filter, true)
}

// ListEnabled returns enabled instances matching the filter, ignoring the healthy flag.
func (p *Pool) ListEnabled(_ context.Context, filter edgedomain.CapabilityFilter) ([]edgedomain.Instance, error) {
	return p.list(filter, false)
}

func (p *Pool) list(filter edgedomain.CapabilityFilter, requireHealthy bool) ([]edgedomain.Instance, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()

	var out []edgedomain.Instance
	for _, e := range p.byID {
		...
		out = append(out, edgedomain.Instance{
			ID:              e.record.ID,
...
func (p *Pool) List() []edgedomain.Instance {
	p.mu.RLock()
	defer p.mu.RUnlock()
	out := make([]edgedomain.Instance, 0, len(p.byID))
	for _, e := range p.byID {
		if e == nil {
			continue
		}
...
func (p *Pool) Get(_ context.Context, id sharedkernel.EdgeID) (*edgedomain.Instance, error) {
	p.mu.RLock()
	defer p.mu.RUnlock()
	e, ok := p.byID[id]
	if !ok || e == nil {
...
func matchCapabilities(have []string, filter edgedomain.CapabilityFilter) bool {
...
var _ edgedomain.Registry = (*Pool)(nil)
