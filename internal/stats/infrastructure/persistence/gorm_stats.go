...
// GormStatsRepository is a GORM-backed statsdomain.Repository.
...
func (r *GormStatsRepository) AddTerminal(ctx context.Context, in statsdomain.AddTerminalInput) error {
	date := statsdomain.DateOf(in.CompletedAt, r.loc)
	now := time.Now().UTC()
	...
	switch in.Status {
	case statsdomain.StatusSucceeded:
		daily.SucceededCount = 1
	case statsdomain.StatusFailed:
		daily.FailedCount = 1
	case statsdomain.StatusCancelled:
	...
	return r.Prune(ctx, statsdomain.DateOf(time.Now().In(r.loc).Add(-r.retention), r.loc))
}

func (r *GormStatsRepository) ListDaily(ctx context.Context, from, to string) ([]statsdomain.DailyRow, error) {
	...
	out := make([]statsdomain.DailyRow, 0, len(rows))
	for _, row := range rows {
		out = append(out, statsdomain.DailyRow{
			Date:            row.StatDate,
...
func (r *GormStatsRepository) ListErrors(ctx context.Context, from, to string, limit int) ([]statsdomain.ErrorRow, error) {
	type item struct {
		ErrorCode string
		Total     int
	}
	...
	out := make([]statsdomain.ErrorRow, 0, len(items))
	for _, it := range items {
		out = append(out, statsdomain.ErrorRow{ErrorCode: it.ErrorCode, Count: it.Total})
	}
	return out, nil
}

func (r *GormStatsRepository) ListEdges(ctx context.Context, from, to string) ([]statsdomain.EdgeRow, error) {
	type item struct {
		EdgeID    string
		Total     int
		Succeeded int
	...
	out := make([]statsdomain.EdgeRow, 0, len(items))
	for _, it := range items {
		out = append(out, statsdomain.EdgeRow{EdgeID: it.EdgeID, Count: it.Total, Succeeded: it.Succeeded, Failed: it.Failed})
	}
	return out, nil
}

func (r *GormStatsRepository) ListCases(ctx context.Context, from, to string, limit int) ([]statsdomain.CaseRow, error) {
	type item struct {
		CaseID          uint64
		CaseName        string
		Total           int
	...
	out := make([]statsdomain.CaseRow, 0, len(items))
	for _, it := range items {
		out = append(out, statsdomain.CaseRow{
			CaseID:          it.CaseID,
...
var _ statsdomain.Repository = (*GormStatsRepository)(nil)
