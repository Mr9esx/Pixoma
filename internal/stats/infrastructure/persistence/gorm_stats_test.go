...
import (
	...
	"github.com/Mr9esx/Pixoma/internal/stats/infrastructure/persistence"
)
...
func TestGormStatsRepository_AddTerminalAndList(t *testing.T) {
	...
	in := statsdomain.AddTerminalInput{
		EdgeID: "gpu-1", ErrorCode: "timeout", Status: statsdomain.StatusFailed,
	...
	if err := repo.AddTerminal(ctx, statsdomain.AddTerminalInput{
		Status: statsdomain.StatusSucceeded, CompletedAt: base, CreatedAt: base,
	}); err != nil {
		t.Fatal(err)
	}
...
func TestGormStatsRepository_Prune(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC()
	for _, in := range []statsdomain.AddTerminalInput{
		{Status: statsdomain.StatusSucceeded, CompletedAt: now.Add(-48 * time.Hour), CreatedAt: now.Add(-48 * time.Hour), CaseID: 9},
		{Status: statsdomain.StatusSucceeded, CompletedAt: now, CreatedAt: now, CaseID: 9},
	} {
		if err := repo.AddTerminal(ctx, in); err != nil {
			t.Fatal(err)
		}
	}
	days, err := repo.ListDaily(ctx, statsdomain.DateOf(now.Add(-72*time.Hour), loc), statsdomain.DateOf(now, loc))
	if err != nil {
		t.Fatal(err)
	}
...
func TestGormStatsRepository_SkipEmptyDimensions(t *testing.T) {
	...
	ctx := context.Background()
	now := time.Now().UTC()
	in := statsdomain.AddTerminalInput{Status: statsdomain.StatusCancelled, CompletedAt: now, CreatedAt: now, CaseID: 2}
	for i := 0; i < 2; i++ {
		if err := repo.AddTerminal(ctx, in); err != nil {
			t.Fatal(err)
		}
	}
	edges, err := repo.ListEdges(ctx, statsdomain.DateOf(now, time.UTC), statsdomain.DateOf(now, time.UTC))
	if err != nil || len(edges) != 0 {
		t.Fatalf("edges should be empty: %+v %v", edges, err)
	}
	errs, err := repo.ListErrors(ctx, statsdomain.DateOf(now, time.UTC), statsdomain.DateOf(now, time.UTC), 10)
	if err != nil || len(errs) != 0 {
		t.Fatalf("errors should be empty: %+v %v", errs, err)
	}
	days, err := repo.ListDaily(ctx, statsdomain.DateOf(now, time.UTC), statsdomain.DateOf(now, time.UTC))
	if err != nil || len(days) != 1 || days[0].Cancelled != 2 {
		t.Fatalf("daily incremental: %+v %v", days, err)
	}
	cases, err := repo.ListCases(ctx, statsdomain.DateOf(now, time.UTC), statsdomain.DateOf(now, time.UTC), 10)
...
func TestDateOfBoundaries(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := statsdomain.DateOf(tc.in, loc); got != tc.want {
