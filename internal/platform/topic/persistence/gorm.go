...
// TopicRepository is a GORM-backed topic.Repository.
...
func (r *TopicRepository) List(ctx context.Context, enabled *bool) ([]topic.Topic, error) {
...
func (r *TopicRepository) Get(ctx context.Context, key string) (*topic.Topic, error) {
	var row TopicRow
...
func toRow(t topic.Topic) TopicRow {
	return TopicRow{
		Key:       t.Key,
		Name:      t.Name,
		Enabled:   t.Enabled,
...
func fromRow(row TopicRow) topic.Topic {
	return topic.Topic{
		Key:       row.Key,
		Name:      row.Name,
		Enabled:   row.Enabled,
