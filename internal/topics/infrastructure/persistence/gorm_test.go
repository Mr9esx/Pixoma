...
import (
	...
	"github.com/Mr9esx/Pixoma/internal/topics/infrastructure/persistence"
)
...
func TestTopicRepository_RoundTrip(t *testing.T) {
	...
	if err := repo.Create(ctx, topicdomain.Topic{Key: "fast-gpu", Name: "Fast GPU", Enabled: true, CreatedAt: now, UpdatedAt: now}); err != nil {
...
func TestTopicRepository_ListFiltersEnabled(t *testing.T) {
	...
	_ = repo.Create(ctx, topicdomain.Topic{Key: "a", Name: "A", Enabled: true, CreatedAt: now, UpdatedAt: now})
	_ = repo.Create(ctx, topicdomain.Topic{Key: "b", Name: "B", Enabled: false, CreatedAt: now, UpdatedAt: now})
...
func TestTopicRepository_CreateDuplicateConflict(t *testing.T) {
	...
	_ = repo.Create(ctx, topicdomain.Topic{Key: "dup", Name: "D", Enabled: true, CreatedAt: now, UpdatedAt: now})
	err := repo.Create(ctx, topicdomain.Topic{Key: "dup", Name: "D2", Enabled: true, CreatedAt: now, UpdatedAt: now})
...
func TestTopicRepository_UpdateAndDelete(t *testing.T) {
	...
	_ = repo.Create(ctx, topicdomain.Topic{Key: "k", Name: "K", Enabled: true, CreatedAt: now, UpdatedAt: now})
	if err := repo.Update(ctx, topicdomain.Topic{Key: "k", Name: "K2", Enabled: false, CreatedAt: now, UpdatedAt: now}); err != nil {
	...
	if _, err := repo.Get(ctx, "k"); !errors.Is(err, topicdomain.ErrTopicNotFound) {
		t.Fatalf("expected ErrTopicNotFound after delete, got %v", err)
	}
	if err := repo.Delete(ctx, "k"); !errors.Is(err, topicdomain.ErrTopicNotFound) {
