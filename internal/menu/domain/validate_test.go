...
func TestValidate_RejectsEmptyTreeDuplicateIDAndPropagatesLookupError(t *testing.T) {
	...
	if err := domain.Validate(ctx, domain.MenuTree{ChannelID: "tg-default", Items: nil}, exists); !errors.Is(err, domain.ErrValidation) {
...
func TestValidate_RejectsDuplicateLabelAndBadReplyMedia(t *testing.T) {
	...
	dup := domain.DefaultSeedTree("tg-default")
	...
	okTree := domain.MenuTree{
		...
		Items: []domain.MenuNode{{
			ID: "parent", Label: "P", Order: 0, Enabled: true, Kind: domain.KindFolder,
			Children: []domain.MenuNode{
				{ID: "child", Label: "Same", Order: 0, Enabled: true, Kind: domain.KindFolder},
			},
		}, {
			ID: "other", Label: "Same", Order: 1, Enabled: true, Kind: domain.KindPlaceholder,
		}},
	}
	...
	emptyReply := domain.DefaultSeedTree("tg-default")
	...
	badURL := domain.DefaultSeedTree("tg-default")
...
func TestValidate_RejectsDuplicateOrderWithinParent(t *testing.T) {
	...
	if err := domain.Validate(ctx, tree, exists); !errors.Is(err, domain.ErrValidation) {
		t.Fatalf("duplicate order within same parent must be rejected, got %v", err)
	}

	// Same order under different parents is allowed.
	ok := domain.MenuTree{
		ChannelID: "tg-default",
		Items: []domain.MenuNode{
			{ID: "a", Label: "A", Order: 0, Enabled: true, Kind: domain.KindFolder,
				Children: []domain.MenuNode{{ID: "a1", Label: "A1", Order: 0, Enabled: true, Kind: domain.KindFolder}}},
			{ID: "b", Label: "B", Order: 1, Enabled: true, Kind: domain.KindPlaceholder},
		},
	}
	if err := domain.Validate(ctx, ok, exists); err != nil {
