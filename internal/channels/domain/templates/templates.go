...
// GlobalDefaultID identifies the platform-default copy scope, shared with
// channel-specific storage adapters.
...
// HasKey reports whether key is a known built-in template key.
func HasKey(key string) bool {
	_, ok := defaults[key]
	return ok
}

// Keys returns all built-in template keys in an unstable order.
func Keys() []string {
...
var defaults = map[string]string{
	KeyWelcome:               "欢迎使用 Pixoma\n选择功能：",
	KeyHelp:                  "选择下方功能。菜单未更新时发送 /menu。",
	KeySelectTemplate:        "选择模板：",
	KeyWorkflowDone:          "工作流已完成\ntask={{ task_id }}",
	KeyWorkflowDoneFollowp:   "还要继续？点菜单再选一个工作流。",
	KeySubmitStarted:         "已提交，生成中… task={{ task_id }}",
	KeyConfirmRun:            "输入完成，确认执行？",
	KeyInputPrompt:           "填写参数 {{ progress }}：输入「{{ key }}」",
	KeyExitDone:              "已退出当前工作流。",
	KeyUnfinishedSession:     "有未完成的工作流，先退出再重新开始。",
	KeyMenuActionPlaceholder: "暂未开放",
	KeyMenuUpdated:           "菜单已更新，使用下方按钮。",
	KeyTaskFailed:            "任务执行失败\ntask={{ task_id }}\n状态：{{ status }}\n{{ error_msg }}",
	KeyTaskCancelled:         "任务已取消\ntask={{ task_id }}",
	KeyPreviewHintLabel:      "预览：",
	KeyButtonStartCase:       "开始工作流",
	KeyInputInvalidNumber:    "输入合法数字，例如 42",
	KeyInputInvalidBoolean:   "输入 true 或 false",
	KeyButtonSkip:            "跳过",
	KeyButtonConfirmRun:      "确认生成",
	KeyButtonExit:            "退出",
	KeySessionTerminated:     "该工作流已被删除，当前会话已结束。",
	KeyAccessDenied:          "当前账号没有使用权限，联系管理员。",
	KeyListTasks:             "我的任务\n\n当前任务\n{{ current }}\n\n最近任务\n{{ recent }}",
}
