...
// BotMessenger renders protocol.Outbound through the Telegram Bot API.
...
func (m *BotMessenger) SendMenu(ctx context.Context, addr sharedkernel.ChannelAddr, title string, _ []protocol.MenuEntry) error {
	chatID, err := externalChatID(addr)
	if err != nil {
		return err
	}
...
func (m *BotMessenger) SendList(ctx context.Context, addr sharedkernel.ChannelAddr, title string, rows [][]protocol.Button) error {
	chatID, err := externalChatID(addr)
	if err != nil {
		return err
	}
...
func (m *BotMessenger) SendMedia(ctx context.Context, addr sharedkernel.ChannelAddr, ref sharedkernel.BlobRef, caption string, buttons [][]protocol.Button) error {
	chatID, err := externalChatID(addr)
	if err != nil {
		return err
	}
...
func (m *BotMessenger) SendMediaURL(ctx context.Context, addr sharedkernel.ChannelAddr, imageURL, mime, caption string, buttons [][]protocol.Button) error {
	chatID, err := externalChatID(addr)
	if err != nil {
		return err
	}
...
func sendUploadByMIME(ctx context.Context, b *bot.Bot, chatID int64, mime, name string, data []byte, caption string, buttons [][]protocol.Button) error {
...
func sendURLByMIME(ctx context.Context, b *bot.Bot, chatID int64, mime, url, caption string, buttons [][]protocol.Button) error {
	truncated := truncateTGText(caption, maxTGCaptionRunes)
	markup := optionalInlineMarkup(buttons)
	switch {
	case strings.HasPrefix(mime, "image/gif"):
...
func (m *BotMessenger) EditReplyMarkup(ctx context.Context, addr sharedkernel.ChannelAddr, messageID int, rows [][]protocol.Button) error {
	chatID, err := externalChatID(addr)
	if err != nil {
		return err
	}
...
func optionalInlineMarkup(rows [][]protocol.Button) models.ReplyMarkup {
	if len(rows) == 0 {
		return nil
	}
...
func toInlineMarkup(rows [][]protocol.Button) *models.InlineKeyboardMarkup {
