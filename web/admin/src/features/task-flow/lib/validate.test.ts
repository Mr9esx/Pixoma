...
describe('validateRule', () => {
  ...
  it('无条件规则合法', () => {
    expect(
      validateRule(okRule({ always: true }) as never, 0, topics, attributes)
    ).toBeNull()
  })
  ...
  it('引用不存在的 Topic 报 topic-unknown', () => {
    const issue = validateRule(
      okRule(undefined, 'ghost-gpu') as never,
      0,
      topics,
      attributes
    )
  ...
  it('引用已禁用 Topic 报 topic-disabled', () => {
    const issue = validateRule(
      okRule(undefined, 'cpu-low') as never,
      0,
      topics,
      attributes
    )
  ...
  it('传入绑定集合且 Topic 已绑定节点时通过', () => {
    ...
    expect(
      validateRule(okRule() as never, 0, topics, attributes, bound)
    ).toBeNull()
  })
})
...
describe('validateRouting', () => {
  it('空配置不合法（没有显式路由）', () => {
    ...
    expect(result.issues[0]?.kind).toBe('routing-empty')
  })
  ...
  it('无条件规则只校验 Topic', () => {
    const result = validateRouting(
      { rules: [okRule({ always: true }) as never] },
      topics,
      attributes
    )
