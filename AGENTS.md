...
项目根目录存在 `.codegraph/` 时，理解或定位代码应先使用 CodeGraph，再按需使用 `rg` 和直接读文件：

- 优先调用 `codegraph_explore`，并传入项目路径；若当前会话没有该 MCP 工具，运行 `codegraph explore "符号、文件名或问题"`。查询已返回的源码无需重复读取。
- 开始依赖索引前运行 `codegraph status .`；若索引过期，先运行 `codegraph sync .`。完成代码改动后运行 `codegraph sync .`，让后续查询使用新索引。
- `.codegraph/` 是本机生成且被 Git 忽略的索引。新工作区没有它时，不要擅自初始化；由用户决定是否运行 `codegraph init .`。
- CodeGraph 查询不准确或未覆盖目标文件时，改用 `rg` 和直接读文件核实。
